...
/*   Updated: 2023/06/11 18:19:23 by ddzuba           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ClapTrap::attack(const std::string &target)
{
	...
	{
		_energy--;
		std::cout << "⚔️ Mechanical Warrior 🤖" << _name << " attacks " << target << ", causing " << _attack
...
void ClapTrap::beRepaired(unsigned int amount)
{
	if (_health <= 0)
	{
		std::cout << "🤖Mechanical Warrior " << _name << " was destroyed!" << DEFAULT << std::endl << std::endl;
		return;
	}
	if (_energy >= 0)
	{
